...
import { ChevronLeft, Clock, Dumbbell, Flame, Link2, ListChecks, Trophy } from 'lucide-react-native';
...
/** Read-only mirror of workouts/detail.tsx, for someone else's completed
 * workout reached from their public profile — no editing affordances at
 * all, since every mutation endpoint it would otherwise reuse (PUT/DELETE
 * /sets, PUT /workouts) is scoped server-side to the viewer's own account
 * and would just 404 here anyway. */
export default function PublicWorkoutDetailScreen() {
  const { ownerId, workoutId } = useLocalSearchParams<{ ownerId: string; workoutId: string }>();
  const router = useRouter();
  const unit = authStore((s) => s.user?.weight_unit) || 'kg';
  const distanceUnit = authStore((s) => s.user?.distance_unit) || 'km';

  const { data: workout, isLoading } = useQuery({
    queryKey: ['users', ownerId, 'workouts', workoutId],
    queryFn: () => api.get<Workout>(`/users/${ownerId}/workouts/${workoutId}`).then((r) => r.data),
    enabled: !!ownerId && !!workoutId,
  });
    ...
    <SafeAreaView style={styles.safeArea} edges={['top']}>
      ...
      <FlatList
        ...
        renderItem={({ item, index }) => {
          ...
          const itemDistanceUnit = item.exercise?.distance_unit || distanceUnit;
              ...
              {item.sets.map((set) => (
                ...
                >
                  ...
                  <Text style={styles.setValue}>
                    {item.isCardio
                      ? `${formatSetDuration(set.duration_seconds || 0)} · ${formatDistance(set.distance_km || 0, itemDistanceUnit)}`
                      : `${formatWeight(set.weight || 0, weightUnit)} × ${set.reps} reps`}
                  </Text>
                  {set.is_warmup ? <Text style={styles.warmupTag}>Calentamiento</Text> : null}
                  {set.superset_group ? (
                    <Link2 size={12} color={colors.accent.ember} strokeWidth={2.2} accessibilityLabel="Serie en superserie" />
                  ) : null}
                  {set.is_pr && (
