...
export default function ExerciseTrashScreen() {
  ...
  const { data: trash, isLoading } = useQuery({
    ...
    queryFn: () => api.get<Exercise[]>('/exercises/trash').then((r) => r.data),
  });
  ...
  const handleRestore = async (id: string) => {
    ...
    try {
      ...
      setError(getErrorMessage(err, 'No se pudo restaurar el ejercicio'));
    } finally {
      setRestoringId(null);
    }
  };
  ...
  const confirmPermanentDelete = async () => {
    ...
    try {
      ...
      Alert.alert('Error', getErrorMessage(err, 'No se pudo eliminar definitivamente'));
    }
  };
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={['top']}>
      <View style={styles.header}>
        ...
        <Text style={styles.title} accessibilityRole="header">Papelera</Text>
      </View>
