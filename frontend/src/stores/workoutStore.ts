  ...
  persist(
    ...
    }),
    {
      ...
      onRehydrateStorage: () => (state, error) => {
        if (error || !state) {
          // Corrupted persisted data, or JSON from an old app version's
          // incompatible schema — `state` is undefined either way, so
          // relying on it to flip hasHydrated would leave it stuck false
          // forever, permanently disabling "start workout" until reinstall.
          workoutStore.getState().setHasHydrated(true);
          return;
        }
