...
export default function AddExerciseScreen() {
  ...
  const { data: routine } = useQuery({
    ...
    queryFn: () => api.get(`/routines/${routineId}`).then((r) => r.data),
    enabled: !!routineId,
  });
  ...
  const handleCreateExercise = async (data: {
    ...
    setSaving(true);
    try {
      const { data: exercise } = await api.post('/exercises', data);
  ...
  const handleAddToRoutine = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'No se pudo añadir el ejercicio');
    } finally {
      setSaving(false);
    }
  };
