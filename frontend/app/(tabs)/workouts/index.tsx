...
export default function WorkoutsScreen() {
  ...
  const { data: routines, isLoading } = useQuery({
    queryKey: ['routines'],
    queryFn: () => api.get('/routines').then((r) => r.data),
  });

  const { data: recentWorkouts } = useQuery({
    queryKey: ['workouts', 'recent'],
    queryFn: () => api.get('/workouts?limit=1').then((r) => r.data),
  });
  const lastWorkout = (recentWorkouts || []).find((w: any) => w.completed_at);

  const { data: lastWorkoutDetail } = useQuery({
    queryKey: ['workouts', lastWorkout?.id],
    queryFn: () => api.get(`/workouts/${lastWorkout.id}`).then((r) => r.data),
    enabled: !!lastWorkout,
  });
