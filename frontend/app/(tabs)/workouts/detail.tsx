...
export default function WorkoutDetailScreen() {
  ...
  const handlePickExercise = (exercise: any) => {
  ...
  const confirmRemoveExercise = async () => {
    ...
    setRemoving(true);
    try {
      const toDelete = (workout?.sets || []).filter((s: any) => s.exercise_id === exerciseId);
  ...
  const exerciseGroups = Object.values(
    sets.reduce(
      (groups: Record<string, { exerciseId: string; name: string; exercise: any; isCardio: boolean; sets: any[] }>, set: any) => {
  ...
  ) as { exerciseId: string; name: string; exercise: any; isCardio: boolean; sets: any[] }[];
  ...
  const rows: { exerciseId: string; name: string; exercise: any; isCardio: boolean; sets: any[] }[] = [
    ...exerciseGroups,
  ...
  const availableExercises = (allExercises || [])
    .filter((e: any) => !addedExerciseIds.has(e.id) && !pendingExercises.some((p) => p.id === e.id))
    .filter((e: any) => e.name.toLowerCase().includes(pickerSearch.trim().toLowerCase()));
